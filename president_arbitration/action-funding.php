...
try {
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as committee_members FROM election_committee_members WHERE status = 'active'");
    $committee_members = $stmt->fetch(PDO::FETCH_ASSOC)['committee_members'];
    
} catch (PDOException $e) {
    error_log("Dashboard statistics error: " . $e->getMessage());
    $total_cases = $pending_cases = $resolved_cases = $upcoming_hearings = $active_elections = $committee_members = 0;
}

// Handle form submissions for new budget request
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit_request'])) {
        ...
        // Get the committee ID for President Arbitration
        try {
            $stmt = $pdo->prepare("SELECT id FROM committee_members WHERE user_id = ? AND role = 'president_arbitration'");
            $stmt->execute([$user_id]);
            $committee_member = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if (!$committee_member) {
                $error = "You are not assigned as President Arbitration in any committee.";
            } else {
...
// Get budget requests data for President Arbitration
try {
    // Get the committee member ID for President Arbitration
    $stmt = $pdo->prepare("SELECT id FROM committee_members WHERE user_id = ? AND role = 'president_arbitration'");
    $stmt->execute([$user_id]);
    $committee_member = $stmt->fetch(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Arbitration President Action & Funding - Isonga RPSU</title>
...
<body>
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                <!-- <div class="page-title">
                    ...
                    <p>Request budget for arbitration committee action plans and track funding status</p>
                </div> -->
                ...
                <div class="card-body">
                    <?php if (empty($budget_requests)): ?>
                        <div style="text-align: center; color: var(--dark-gray); padding: 3rem;">
                            ...
                            <p>Submit your first funding request to get started with your arbitration committee action plans.</p>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    <a href="view_budget_request.php?id=<?php echo $request['id']; ?>" class="btn btn-sm btn-outline">
                                                        <i class="fas fa-eye"></i> View
                                                    </a>
                                                    <?php if (!empty($request['action_plan_file_path'])): ?>
                                                        <a href="../<?php echo $request['action_plan_file_path']; ?>" class="btn btn-sm btn-primary" title="Download File" download>
                                                            <i class="fas fa-download"></i>
                                                        </a>
                                                    <?php endif; ?>
                                                    <?php if ($request['status'] === 'approved_by_president'): ?>
                                                        <a href="generate_approval_letter.php?id=<?php echo $request['id']; ?>" class="btn btn-sm btn-success" title="Download Approval Letter">
            ...
            <form method="POST" enctype="multipart/form-data" id="budgetRequestForm">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="request_title" placeholder="e.g., Arbitration Workshop Materials Budget" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="purpose" rows="4" placeholder="Describe the purpose of this funding and how it will be used for arbitration activities..." required></textarea>
                    </div>
    ...
    <script>
        ...
        setInterval(() => {
            console.log('Action & Funding page auto-refresh triggered');
        }, 180000);
    </script>
</body>
</html>
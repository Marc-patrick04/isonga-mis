...
try {
    ...
    $pdf->SetTitle('Arbitration President Budget Approval Letter');
    ...
    $pdf->SetTextColor(220, 53, 69); // Red color for arbitration
    $pdf->Cell(0, 10, 'ARBITRATION PRESIDENT PROGRAM', 0, 1, 'C');
    $pdf->SetFont('helvetica', 'B', 16);
    ...
    $pdf->Cell(0, 10, 'Ref: RPSU/MC/ARBP/' . date('Y') . '/' . str_pad($request_id, 4, '0', STR_PAD_LEFT), 0, 1, 'R');
    ...
    // Introduction
    $intro = "I, " . $request['requester_name'] . ", serving as President of Arbitration at RP Musanze College, hereby strongly prove that I requested a budget of RWF " . 
    ...
    $purpose = "This letter serves as official evidence and reference for the Arbitration President budget approved to support: " . 
        $request['purpose'] . ". The funds will be utilized for arbitration activities, conflict resolution programs, " .
        "and dispute settlement operations as stated in the application.\n\n";

    $pdf->MultiCell(0, 8, $purpose, 0, 'J');

    // Financial Details Table
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->Cell(0, 10, 'ARBITRATION BUDGET DETAILS:', 0, 1);
    ...
    <tr style="background-color:#f2f2f2;">
        <td><b>Total Approved for Arbitration</b></td>
    ...
    $pdf->Cell(0, 10, 'GUIDELINES FOR ARBITRATION BUDGET UTILIZATION:', 0, 1, 'L', true);
    $pdf->Ln(3);

    $pdf->SetFont('helvetica', '', 11);
    $guidelines = "1. The approved amount MUST be used exclusively for: " . $request['purpose'] . "\n" .
        "2. Funds allocated for arbitration proceedings, mediation activities, and conflict resolution\n" .
        "3. Budget must support fair and impartial dispute settlement processes\n" .
        ...
        "9. Submit detailed expenditure report with conflict resolution outcomes within 30 days\n" .
    ...
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->SetFillColor(220, 53, 69); // Red background for arbitration
    ...
    $disbursement = "To receive the approved Arbitration budget, you MUST present this printed letter at the Guild Council Office along with:\n\n" .
        "✓ Original Student ID Card\n" .
        "✓ Printed copy of this approval letter\n" .
        "✓ Completed disbursement form (available at the office)\n" .
        "✓ Detailed arbitration activity implementation plan\n" .
        "✓ Conflict resolution program assessment plan\n\n" .
    ...
    // Agreement Section
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->Cell(0, 10, 'ARBITRATION PRESIDENT DECLARATION AND AGREEMENT:', 0, 1);
    $pdf->SetFont('helvetica', '', 11);

    $agreement = "I hereby declare that I have read and understood all the terms and conditions stated above. " .
        "As President of Arbitration, I agree to use the approved budget responsibly for conflict resolution, " .
        "mediation activities, and arbitration proceedings that benefit the entire student community. I acknowledge that " .
    ...
    <tr>
        ...
        <!-- RIGHT CELL: ARBITRATION PRESIDENT ACKNOWLEDGEMENT -->
        <td width="50%" class="signature-cell">
            <div class="signature-header">ARBITRATION PRESIDENT ACKNOWLEDGEMENT</div>
            <div style="margin-bottom: 10px; font-size: 11px;">
                I acknowledge receipt of arbitration budget: RWF <strong>' . number_format($approved_amount, 2) . '</strong>
            </div>
            <div style="margin-bottom: 8px; font-size: 11px;">
                Name: ' . $request['requester_name'] . '
            </div>
            <div style="margin-bottom: 8px; font-size: 11px;">
                Position: President of Arbitration
            </div>
            <div style="margin-bottom: 8px; font-size: 11px;">
    ...
    // Justice and Accountability Section
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->SetFillColor(255, 245, 245); // Light red background
    $pdf->Cell(0, 10, 'JUSTICE AND ACCOUNTABILITY DECLARATION', 0, 1, 'C', true);
    $pdf->SetFont('helvetica', '', 10);
    $pdf->Ln(3);
    
    $accountability_text = "As President of Arbitration, I commit to:\n" .
        "✓ Impartial and fair utilization of funds for justice administration\n" .
        "✓ Transparent reporting on arbitration budget utilization\n" .
        "✓ Upholding the highest standards of judicial integrity\n" .
        "✓ Ensuring arbitration activities promote campus harmony\n" .
        "✓ Protecting students' rights through proper dispute resolution\n\n" .
        "Signature: __________________________ Date: __________________";
    
    $pdf->MultiCell(0, 8, $accountability_text, 0, 'L');
    $pdf->Ln(10);

    // Arbitration Specific Notes
    $pdf->SetFont('helvetica', 'I', 11);
    $pdf->SetFillColor(245, 245, 255); // Light blue background
    $pdf->MultiCell(0, 8, 
        "Note: This budget is specifically allocated for arbitration and conflict resolution activities. " .
        "As President of Arbitration, you are responsible for ensuring these funds promote justice, fairness, " .
        "and peaceful resolution of disputes within the student community. All activities funded must align " .
        "with the principles of impartiality and due process.", 
        0, 'L', true
    );
    $pdf->Ln(10);
    ...
    $pdf->SetTextColor(128, 128, 128); // Gray color
    $pdf->MultiCell(0, 8, 
        "Note: This document serves as official approval for Arbitration President budget disbursement. " .
        "Any alterations to this document render it invalid. As a judicial leader, you are accountable to " .
        "the student body for transparent and responsible use of these funds in promoting campus justice.\n\n" .
    ...
    $pdf->Text(15, 150, 'JUSTICE');
    $pdf->Text(15, 230, 'ARBITRATION');
    $pdf->SetAlpha(1);

    // Output PDF
    $filename = 'Arbitration_President_Budget_Approval_' . ($request['requester_reg'] ?: 'President_Arb') . '_' . date('Ymd') . '.pdf';

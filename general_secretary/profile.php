...
try {
    ...
    // Get unread messages from conversation_participants
    $stmt = $pdo->prepare("
            ...
            case 'update_profile':
                ...
                $stmt = $pdo->prepare("
                    ...
                    SET full_name = ?, email = ?, phone = ?, date_of_birth = ?, gender = ?, 
                        ...
                        department_id = ?, program_id = ?, updated_at = NOW()
                    WHERE id = ?
                ");
                $stmt->execute([
...
try {
    $deptStmt = $pdo->query("SELECT id, name FROM departments WHERE is_active = 1 ORDER BY name");
    $departments = $deptStmt->fetchAll(PDO::FETCH_ASSOC);
    
    $progStmt = $pdo->query("SELECT id, name FROM programs WHERE is_active = 1 ORDER BY name");
...
<body>
    ...
    <div class="dashboard-container">
        <!-- Sidebar - General Secretary Specific -->
        <nav class="sidebar">
                                ...
                                <div class="form-section">
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 1rem; background: var(--light-gray); border-radius: var(--border-radius);">
                                        ...
                                        <form method="POST" style="margin: 0;">
                                            ...
                                            <button type="submit" class="btn <?php echo $user['two_factor_enabled'] ? 'btn-danger' : 'btn-success'; ?>">

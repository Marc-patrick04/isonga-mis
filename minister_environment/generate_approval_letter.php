...
try {
    ...
    $pdf->SetTitle('Environment & Security Budget Approval Letter');
    ...
    $pdf->SetTextColor(0, 128, 0); // Green color for environment
    $pdf->Cell(0, 10, 'ENVIRONMENT & SECURITY BUDGET PROGRAM', 0, 1, 'C');
    $pdf->SetFont('helvetica', 'B', 16);
    ...
    $pdf->Cell(0, 10, 'Ref: RPSU/MC/ESBP/' . date('Y') . '/' . str_pad($request_id, 4, '0', STR_PAD_LEFT), 0, 1, 'R');
    ...
    // Introduction
    $intro = "I, " . $request['requester_name'] . ", serving as Minister of Environment & Security at RP Musanze College, hereby strongly prove that I requested a budget of RWF " . 
    ...
    $purpose = "This letter serves as official evidence and reference for the environment & security budget approved to support: " . 
        $request['purpose'] . ". The funds will be utilized for environmental sustainability initiatives, campus security enhancements, " .
        "and safety improvements as stated in the application.\n\n";
    ...
    $pdf->Cell(0, 10, 'GUIDELINES FOR ENVIRONMENT & SECURITY BUDGET UTILIZATION:', 0, 1, 'L', true);
    $pdf->Ln(3);

    $pdf->SetFont('helvetica', '', 11);
    $guidelines = "1. The approved amount MUST be used exclusively for: " . $request['purpose'] . "\n" .
        "2. Funds allocated for environmental sustainability, campus security, and safety initiatives\n" .
        "3. Environmental projects must include sustainability impact assessment\n" .
        "4. Security equipment purchases must follow college procurement procedures\n" .
        "5. Keep all original receipts and invoices for verification purposes\n" .
        ...
        "10. Submit detailed expenditure report with environmental/security impact assessment within 30 days\n\n";

    $pdf->MultiCell(0, 8, $guidelines, 0, 'L');

    // Disbursement Instructions
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->SetFillColor(0, 128, 0); // Green background for environment
    ...
    $disbursement = "To receive the approved environment & security budget, you MUST present this printed letter at the Guild Council Office along with:\n\n" .
        "✓ Original Student ID Card\n" .
        "✓ Printed copy of this approval letter\n" .
        "✓ Completed disbursement form (available at the office)\n" .
        "✓ Detailed environment & security implementation plan\n" .
        "✓ Environmental sustainability impact assessment (for environment projects)\n" .
        "✓ Security risk assessment (for security projects)\n\n" .
    ...
    $agreement = "I hereby declare that I have read and understood all the terms and conditions stated above. " .
        "I agree to use the approved environment & security budget responsibly for environmental sustainability " .
        "initiatives and campus security enhancements. I acknowledge that any violation of these terms may result " .
        "in disciplinary action and requirement to repay misused funds.\n";
    ...
    <tr>
        ...
        <td width="50%" class="signature-cell">
            ...
            <div style="margin-bottom: 8px; font-size: 11px;">
                Position: Minister of Environment & Security
            </div>
            <div style="margin-bottom: 8px; font-size: 11px;">
    ...
    // Environment & Security Compliance Section
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->SetFillColor(240, 255, 240); // Light green background
    $pdf->Cell(0, 10, 'ENVIRONMENT & SECURITY COMPLIANCE CERTIFICATION', 0, 1, 'C', true);
    $pdf->SetFont('helvetica', '', 10);
    $pdf->Ln(3);
    
    $compliance_text = "I certify that the proposed environment & security initiative complies with:\n" .
        "✓ RP Musanze College Environmental Sustainability Policy\n" .
        "✓ Campus Security and Safety Regulations\n" .
        "✓ Students Guild Financial Guidelines\n" .
        "✓ College Procurement Procedures\n" .
        "✓ Waste Management and Recycling Standards\n" .
        "✓ Green Campus Initiative Requirements\n\n" .
    ...
    $pdf->SetTextColor(128, 128, 128); // Gray color
    $pdf->MultiCell(0, 8, 
        "Note: This document serves as official approval for environment & security budget disbursement. " .
        "Any alterations to this document render it invalid. Environmental and security projects must adhere " .
        "to college policies and contribute to sustainable campus development and safety.\n\n" .
    ...
    $pdf->Text(15, 150, 'GREEN');
    $pdf->Text(15, 230, 'CAMPUS');
    $pdf->SetAlpha(1);

    // Output PDF
    $filename = 'Environment_Security_Budget_Approval_' . ($request['requester_reg'] ?: 'Minister_ES') . '_' . date('Ymd') . '.pdf';

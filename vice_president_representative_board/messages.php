...
try {
    // Class representatives count
    $stmt = $pdo->query("SELECT COUNT(*) as total_reps FROM committee_members WHERE role = 'class_representative' AND status = 'active'");
    $total_reps = $stmt->fetch(PDO::FETCH_ASSOC)['total_reps'];
    
    // Assigned reports count
    $stmt = $pdo->prepare("
        SELECT COUNT(*) as assigned_reports 
        FROM reports 
        WHERE assigned_to = ? AND status IN ('submitted', 'in_review')
    ");
    $stmt->execute([$user_id]);
    $assigned_reports = $stmt->fetch(PDO::FETCH_ASSOC)['assigned_reports'];
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE mp.user_id = ? AND m.meeting_date >= CURDATE() AND m.status = 'scheduled'
    ");
    $stmt->execute([$user_id]);
    $upcoming_meetings = $stmt->fetch(PDO::FETCH_ASSOC)['upcoming_meetings'];
    
    // Assigned student issues count
    $stmt = $pdo->prepare("
        SELECT COUNT(*) as assigned_issues 
        FROM tickets 
        WHERE assigned_to = ? AND status IN ('open', 'in_progress')
    ");
    $stmt->execute([$user_id]);
    $assigned_issues = $stmt->fetch(PDO::FETCH_ASSOC)['assigned_issues'];
    ...
    $total_reps = $assigned_reports = $upcoming_meetings = $assigned_issues = $unread_messages = 0;
}
...
// Get committee members and class representatives for new conversation
try {
    $members_stmt = $pdo->query("
        SELECT id, full_name, role, department_id 
        FROM users 
        WHERE status = 'active' 
        AND (role IN ('president_representative_board', 'class_representative', 'guild_president', 'vice_guild_academic', 'vice_guild_finance', 'minister_sports', 'minister_health', 'minister_environment', 'minister_culture', 'minister_gender', 'general_secretary')
             OR role LIKE '%executive%')
        ORDER BY 
            CASE 
                WHEN role = 'president_representative_board' THEN 1
                WHEN role = 'class_representative' THEN 2
                ELSE 3
            END,
            full_name
    ");
    ...
    try {
        switch ($action) {
            ...
            case 'create_announcement':
                $participants = $_POST['announcement_participants'] ?? [];
                $announcement_title = trim($_POST['announcement_title']) ?: 'Important Announcement';
                $announcement_content = trim($_POST['announcement_content']);
                
                if (!empty($participants) && !empty($announcement_content)) {
                    ...
                    // Add announcement as first message
                    $message_stmt = $pdo->prepare("
                        INSERT INTO conversation_messages (conversation_id, sender_id, content) 
                        VALUES (?, ?, ?)
                    ");
                    $message_stmt->execute([$new_conversation_id, $user_id, $announcement_content]);
                    
                    $_SESSION['success'] = "Announcement sent successfully!";
                    
                    // Redirect to new announcement
                    header("Location: messages.php?conversation=$new_conversation_id");
                    exit();
                }
                break;
...
<head>
    ...
    <title>Messages - Vice President Representative Board - Isonga RPSU</title>
...
<body>
        ...
        <div class="nav-container">
            <div class="logo-section">
                ...
                <div class="brand-text">
                    <h1>Isonga - Vice President Representative Board</h1>
                </div>
            </div>
            <div class="user-menu">
            ...
            <div class="messages-container">
                ...
                <div class="conversations-sidebar">
                    <div class="sidebar-header">
                        <h2>Representative Board Communications</h2>
                    ...
                    <div class="conversation-list">
                        <?php if (empty($conversations)): ?>
                            <div class="empty-state">
                                ...
                                <p style="font-size: 0.8rem; margin-top: 0.5rem;">Start a new chat with committee members or class representatives</p>
                            </div>
                        <?php else: ?>
    ...
    <div id="newConversationModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>New Representative Board Conversation</h3>
    ...
    <div id="newAnnouncementModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>New Representative Board Announcement</h3>
                <span class="close">&times;</span>
            </div>
            <div class="modal-body">
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="announcement_content" name="announcement_content" class="form-control" 
                                  placeholder="Type your announcement here..." rows="5" required></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="participant-list">
                            <?php foreach ($committee_members as $member): ?>
                                <div class="participant-item">
                                    <input type="checkbox" name="announcement_participants[]" value="<?php echo $member['id']; ?>" 
                                           id="announcement_participant_<?php echo $member['id']; ?>">
                                    <div class="participant-info">
                                        <div class="participant-name"><?php echo htmlspecialchars($member['full_name']); ?></div>
                                        <div class="participant-role">
                                            <?php echo str_replace('_', ' ', $member['role']); ?>

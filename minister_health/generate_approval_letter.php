...
try {
    ...
    $pdf->SetTitle('Health Budget Approval Letter');
    $pdf->SetSubject('Health Budget Approval');
    ...
    $pdf->SetTextColor(0, 102, 0); // Green color for health
    $pdf->Cell(0, 10, 'HEALTH BUDGET PROGRAM', 0, 1, 'C');
    $pdf->SetFont('helvetica', 'B', 16);
    ...
    $pdf->Cell(0, 10, 'Ref: RPSU/MC/HBP/' . date('Y') . '/' . str_pad($request_id, 4, '0', STR_PAD_LEFT), 0, 1, 'R');
    ...
    // Introduction
    $intro = "I, " . $request['requester_name'] . ", serving as Minister of Health at RP Musanze College, hereby strongly prove that I requested a health budget of RWF " . 
    ...
    $purpose = "This letter serves as official evidence and reference for the health budget approved to support: " . 
        $request['purpose'] . ". The funds will be utilized for health initiatives, wellness programs, medical supplies, " .
        "and campus health promotion activities as stated in the application.\n\n";

    $pdf->MultiCell(0, 8, $purpose, 0, 'J');

    // Financial Details Table
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->Cell(0, 10, 'HEALTH BUDGET DETAILS:', 0, 1);
    ...
    $pdf->Cell(0, 10, 'GUIDELINES FOR HEALTH BUDGET UTILIZATION:', 0, 1, 'L', true);
    $pdf->Ln(3);

    $pdf->SetFont('helvetica', '', 11);
    $guidelines = "1. The approved amount MUST be used exclusively for: " . $request['purpose'] . "\n" .
        "2. Funds allocated for medical supplies, health equipment, wellness programs, and health campaigns\n" .
        "3. All health initiatives must comply with campus health and safety regulations\n" .
        "4. Medical equipment purchases must follow college procurement procedures\n" .
        "5. Keep all original receipts, medical purchase orders, and health service invoices\n" .
        ...
        "10. Submit a detailed health impact report within 30 days after fund utilization\n" .
        "11. Health campaigns must include preventive measures and educational components\n" .
        "12. This approval is non-transferable to any other committee or purpose\n\n";

    $pdf->MultiCell(0, 8, $guidelines, 0, 'L');

    // Health Project Requirements
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->SetFillColor(0, 102, 0); // Green background for health
    $pdf->SetTextColor(255, 255, 255); // White text
    $pdf->Cell(0, 10, 'HEALTH PROJECT REQUIREMENTS:', 0, 1, 'L', true);
    $pdf->Ln(3);

    $pdf->SetFont('helvetica', '', 11);
    $pdf->SetTextColor(0, 0, 0); // Black text
    $health_requirements = "✓ All health projects must include health impact assessment\n" .
        "✓ Health initiatives must comply with campus health regulations and policies\n" .
        "✓ Regular progress reports must be submitted to the Students Guild Health Committee\n" .
        "✓ Final project completion report with health outcomes is mandatory\n" .
        "✓ Medical supplies must be properly stored and documented\n" .
        "✓ Health campaigns must target identified campus health priorities\n" .
        "✓ Collaboration with college health services is encouraged\n" .
        "✓ Emergency response plans must be included where applicable\n\n";

    $pdf->MultiCell(0, 8, $health_requirements, 0, 'L');
    ...
    $pdf->SetFont('helvetica', '', 11);
    $pdf->SetTextColor(0, 0, 0); // Black text
    $disbursement = "To receive the approved health budget, you MUST present this printed letter at the Guild Council Office along with:\n\n" .
        "✓ Original Student ID Card\n" .
        "✓ Printed copy of this approval letter\n" .
        "✓ Completed disbursement form (available at the office)\n" .
        "✓ Detailed health initiative implementation plan\n" .
        "✓ Health impact assessment report (if required)\n\n" .
    ...
    // Agreement Section
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->Cell(0, 10, 'MINISTER OF HEALTH DECLARATION AND AGREEMENT:', 0, 1);
    $pdf->SetFont('helvetica', '', 11);

    $agreement = "I hereby declare that I have read and understood all the terms and conditions stated above. " .
        "I agree to use the approved health budget responsibly for campus health initiatives, wellness programs, " .
        "and medical support activities. I acknowledge that any violation of these terms may result in " .
        "disciplinary action and requirement to repay misused funds.\n";
    ...
    <tr>
        ...
        <!-- RIGHT CELL: MINISTER OF HEALTH ACKNOWLEDGEMENT -->
        <td width="50%" class="signature-cell">
            <div class="signature-header">MINISTER OF HEALTH ACKNOWLEDGEMENT</div>
            <div style="margin-bottom: 10px; font-size: 11px;">
                I acknowledge receipt of health budget: RWF <strong>' . number_format($approved_amount, 2) . '</strong>
            </div>
            <div style="margin-bottom: 8px; font-size: 11px;">
    ...
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->SetFillColor(0, 102, 0, 0.1); // Light green background
    $pdf->Cell(0, 10, 'HEALTH COMPLIANCE CERTIFICATION', 0, 1, 'C', true);
    $pdf->SetFont('helvetica', '', 11);
    
    $compliance_text = "I certify that the proposed health initiative complies with:\n\n" .
        "✓ RP Musanze College Health and Safety Policy\n" .
        "✓ Campus Health Regulations and Guidelines\n" .
        "✓ Students Guild Financial Management Guidelines\n" .
        "✓ College Procurement and Medical Equipment Procedures\n" .
        "✓ National Health Standards and Protocols\n" .
        "✓ Emergency Response and First Aid Requirements\n\n" .
        "Signature: __________________________ Date: __________________";
    
    $pdf->MultiCell(0, 8, $compliance_text, 0, 'L');
    $pdf->Ln(10);
    ...
    <table border="1" cellpadding="8" cellspacing="0" width="100%" style="border-collapse: collapse; font-size: 10px;">
        <tr style="background-color:#f2f2f2;">
            ...
            <td width="25%" align="center"><strong>Health Budget Disbursed</strong></td>
    ...
    $pdf->SetTextColor(128, 128, 128); // Gray color
    $pdf->MultiCell(0, 8, 
        "Note: This document serves as official approval for health budget disbursement. " .
        "Any alterations to this document render it invalid. Please keep this letter for your records " .
        "and present it when required for verification purposes. Health projects must adhere to " .
        "college health policies and contribute to campus wellness and prevention initiatives.\n\n" .
    ...
    $pdf->Text(15, 150, 'HEALTH');
    $pdf->Text(15, 230, 'BUDGET');
    $pdf->SetAlpha(1);

    // Output PDF
    $filename = 'Health_Budget_Approval_' . ($request['requester_reg'] ?: 'Minister_Health') . '_' . date('Ymd') . '.pdf';
